...
# --------------------------------------------------------------------------- #
# Gate: proactive preemptions are reported to the model runner
# --------------------------------------------------------------------------- #
...
def test_proactive_preemption_is_reported_in_scheduler_output():
    ...
    low = make_request("low", num_tokens=4, max_tokens=200, arrival_time=1.0)
    scheduler.add_request(low)
    for _ in range(7):  # demote the running call below Q1
        _step(scheduler)
    assert low.status == RequestStatus.RUNNING
...
def test_v2_worker_slot_accounting_never_overflows_under_churn():
    ...
    calls). Sustained fresh Q1 arrivals against a full batch of demoted calls
    keeps the proactive-preemption path hot the whole run.
    """
    max_num_seqs = 2
    scheduler = create_mlfq_scheduler(max_num_seqs=max_num_seqs)
    ...
    for i in range(2):
        scheduler.add_request(
            make_request(f"long{i}", num_tokens=4, max_tokens=400, arrival_time=0.0)
        )
    for _ in range(8):  # demote the long calls so fresh calls outrank them
        step_with_slot_accounting()

    for i in range(200):
        scheduler.add_request(
            make_request(f"f{i}", num_tokens=4, max_tokens=2, arrival_time=8.0 + i)
        )
        step_with_slot_accounting()
