...
def test_old_program_completes_under_sustained_overload():
    ...
    Sustained fresh-program arrivals (2 demanded tokens/step) exceed capacity
    (1 token/step, max_num_seqs=1), so fresh bin-0 calls are always waiting --
    the exact regime that starved old programs unboundedly before the beta
    guard (v5/BEAM overload collapse, POLICY_REFERENCE.md §6a). The old
    program (T_p=10, bin 3) must first starve (no service while fresh calls
    win) and then be promoted at W ~= beta * T_p and complete.
    """
    scheduler = create_plas_scheduler(max_num_seqs=1)
    scheduler.process_table.get_or_create("OLD").service = 10.0
